...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    if File.file?(File.join(Msf::Config.loot_directory, datastore['ASSEMBLY']))
      ...
      print_error("#{datastore['ASSEMBLY']} not found")
      return
    end
    ...
    if params_size > 1023
      print_error('Parameters max lenght 1024 ')
      return
    end
    ...
    if installed_dotnet_versions == []
      print_error('Target has no .NET framework installed')
      return
    end

    exe_clr_version = find_required_clr(exe_path)
    print_status("Assembly CLR version required #{exe_clr_version}")

    if check_requirements(exe_clr_version, installed_dotnet_versions) == false
      print_error('CLR required for assembly not installed')
      return
    end
    ...
    if process == nil
      print_error("Fail to open process on target")
      return
    end
    ...
    if datastore['PID'] <= 0
      ...
      print_status("Run finish and do not read output")
  ...
  def execute_assembly(exe_path)
    ...
    if process == nil
      print_bad("Fail to open process on target")
      return
    end
